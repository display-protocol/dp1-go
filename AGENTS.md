...
- **Merge package:** `merge` applies resolution order: defaults → item `inlineManifest` controls → ref manifest controls → item `override` → item-local fields. The `inlineManifest` slot sits immediately below `ref` per Playlist Extension §3.6 (a fetched manifest is authoritative; the inline copy is the offline fallback). Changes must preserve that ordering and remain consistent with README and spec intent.
