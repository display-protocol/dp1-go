...
// scheduledPlaylist builds a playlist whose displayAt fields activate scheduling (§3.5.1).
...
func TestResolveDynamicQuery_ThenSchedule_Section356(t *testing.T) {
  ...
  "items":[
	...
	wantMerged := []string{staticID, pastID, futureID, absentID, calID}
	gotMerged := itemIDs(out.Items)
	if len(gotMerged) != len(wantMerged) {
		t.Fatalf("merged items got %v, want %v", gotMerged, wantMerged)
	}
	for i := range wantMerged {
		if gotMerged[i] != wantMerged[i] {
			t.Fatalf("merged items got %v, want %v", gotMerged, wantMerged)
		}
	}
	for _, it := range out.Items {
		if it.ID == calID {
			if it.DisplayAt == nil || *it.DisplayAt != "2026-02-30T00:00:00" {
				t.Fatalf("calendar-invalid displayAt not preserved on merged list: %+v", it.DisplayAt)
			}
		}
	}
...
func TestComputeActiveSet_WithoutDisplayAtReturnsAll(t *testing.T) {
	t.Parallel()

	// §3.5.1: no item-level displayAt → full list; scheduling is inactive.
	p := &playlist.Playlist{
		Items: []playlist.PlaylistItem{
	...
	active := ComputeActiveSet(p, now, time.UTC)
	if len(active) != 2 {
		t.Fatalf("got %d items, want 2", len(active))
	}
}

func TestNextDisplayAt_WithoutDisplayAtReturnsNil(t *testing.T) {
	t.Parallel()

	// §3.5.1: no item-level displayAt → no scheduling timers.
	items := []playlist.PlaylistItem{
		makeItem("1", "A", "https://a.com/a", ""),
		makeItem("2", "B", "https://a.com/b", ""),
	}
