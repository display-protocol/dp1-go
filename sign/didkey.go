...
func Ed25519PublicKeyFromDIDKey(kid string) (ed25519.PublicKey, error) {
	if len(kid) < len(didKeyPrefix) || !strings.EqualFold(kid[:len(didKeyPrefix)], didKeyPrefix) {
		return nil, fmt.Errorf("%w: kid must use did:key form", ErrSigInvalid)
	}
	mbStr := kid[len(didKeyPrefix):]
	if mbStr == "" {
		return nil, fmt.Errorf("%w: empty did:key method-specific id", ErrSigInvalid)
	}
	enc, data, err := mb.Decode(mbStr)
	if err != nil {
		return nil, fmt.Errorf("%w: decode did:key multibase: %w", ErrSigInvalid, err)
	}
	if enc != mb.Base58BTC {
		return nil, fmt.Errorf("%w: did:key must use multibase base58btc", ErrSigInvalid)
	}
	if len(data) != 2+ed25519.PublicKeySize {
		return nil, fmt.Errorf("%w: did:key key material length", ErrSigInvalid)
	}
	if data[0] != 0xed || data[1] != 0x01 {
		return nil, fmt.Errorf("%w: did:key is not ed25519-pub", ErrSigInvalid)
	}
